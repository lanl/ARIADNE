# this module contains the classes for n, gamma cross sections experiments

class ngammacs(object):
    # mother class ngammacs
    ...
    def __init__(self,general_info,data,unc_iso,features):
        ...
        # WIP: error message if not n,gamma
        # WIP: error message if n,gamma and Einc = 0
        ...
        if 'en_rsl' in unc_iso['einc_unc']:
            raise ValueError("en_rsl is no longer a valid arguement")
        ...
        if 'tof_t0' in unc_iso['einc_unc']:
            assert hasattr(self, 'tof_length_val'), "Need tof length to propagate tof_t0 uncertainty"
       ...
       #  -------------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------------
        # Add option to load partial covariance matrix ----------------------------------
        if 'partialcov' in unc_iso:
            self.cov_part = unc_iso['partialcov']
        else: 
            self.cov_part = zeros([self.no_val,self.no_val],dtype = float)
    ...
    def calculate_cov_iso(self):
        ...
        for cor_type in self.unc_iso_type:
            ...
            if self.unc_iso_unit[no_unc] != '%':
                raise ValueError("Unit conversion in cov_iso not implemented!")
...
# n, gamma cross section absolute data ########################################################################    
class ngammacs_absolute(ngammacs):
    def __init__(self,general_info,data,unc_iso,features):
        ...
        super(ngammacs_absolute,self).__init__(general_info=general_info,data=data,unc_iso=unc_iso,features=features)
        ...
        self.cov_iso = super(ngammacs_absolute,self).calculate_cov_iso()
        ...
        if self.no_val == 1:
            partial_unc = array([self.einc,self.data_total['data'],self.data_total['rel_unc'][0]*100,sqrt(self.cov_iso[0,0]),\
                                 sqrt(self.cov_norm),sqrt(self.cov_eunc[0,0]),sqrt(self.cov_part[0,0])])
        else:
            partial_unc = array([self.einc,self.data_total['data'],self.data_total['rel_unc']*100,sqrt(diag(self.cov_iso)),\
                             sqrt(diag(self.cov_norm)),sqrt(diag(self.cov_eunc)),sqrt(diag(self.cov_part))])
    ...
    def get_data_cor_rel(self):
        ...
        # get cor ---------------------------------------------------------------
        cov = self.cov_eunc + self.cov_norm + self.cov_iso + self.cov_part
    ...
    def plot_data(self):
        ...
        from matplotlib.pyplot import plot,semilogx,legend,xlabel,ylabel,figure,\
            savefig,errorbar, xscale, yscale, rcParams, yscale, xlim, ylim, loglog, xticks, yticks
        ...
        ylabel(self.isotope + ' n, gamma Cross Section ('+self.values_unit+')')
        ...
        xlabel('Incident Neutron Energy ('+self.einc_unit+')')
        ylabel(self.isotope + ' n, gamma Cross Section ('+self.values_unit+')')
        savefig(self.general_info['output_folder']+'Data_log_truncated.eps') 
        # -----------------------------------------------------------------------
        ...
        figcount +=1
        if self.no_val > 1:
            semilogx(self.einc,self.data_total['rel_unc']*100,'b-.')
            ...
            legend(['Total','Isotope','TargetArealDensity','Eunc'],\
        ...
        figcount +=1
        if self.no_val > 1:
            plot(self.einc,self.data_total['rel_unc']*100,'b-.')
...
class ngammacs_shape(ngammacs):
    def __init__(self,general_info,data,unc_iso,features):
        ...
        # storing data in appropriate containers -----------------------------------------
        # loading data values, isotope uncertainties and general information in
        super(ngammacs_shape,self).__init__(general_info=general_info,data=data,unc_iso=unc_iso,features=features)
        ...
        # calculating the different uncertainty types ------------------------------------        
        # WIP see if all units are the same, i.e. percent
        self.cov_iso = super(ngammacs_shape,self).calculate_cov_iso()
        self.cov_eunc = calculate_cov_eunc_NORATIO(self)
...
# n, gamma cross section absolute data ########################################################################    
class ngammacs_cleanratioabsolute(ngammacs):
    def __init__(self,general_info,data,unc_iso,reference,features):
        ...
        # storing data in appropriate containers -----------------------------------------
        # loading data values, isotope uncertainties and general information in
        super(ngammacs_cleanratioabsolute,self).__init__(general_info=general_info,data=data,unc_iso=unc_iso,features=features)
        ...
        # WIP see if all units are the same, i.e. percent
        self.cov_norm = self.calculate_cov_norm()
        self.cov_iso = super(ngammacs_cleanratioabsolute,self).calculate_cov_iso()
    ...
    def get_cov_refnum(self):
       ...
       # check whether lattice of standard encompasses the exp. lattice ........
        ...
        ylabel(self.reference['isotope'] + ' (n,gamma) cs ('+dataunccor['data_unit']+')')
    ...
    def plot_data(self):
        ...
        """        
        from matplotlib.pyplot import plot,semilogx,legend,xlabel,ylabel,figure,\
            savefig,errorbar, xscale ,rcParams, xticks, yticks
        from numpy import diag, sqrt, meshgrid
        from pylab import pcolormesh, colorbar, show, title, imsave
        from MatrixFunctions import get_cor
        ...
        xlabel('Incident Neutron Energy ('+self.einc_unit+')')
        ylabel(self.isotope + '/'+self.reference['isotope']+ ' (n,gamma) cs')
        ...
        if self.no_val > 1:
            ...
            semilogx(self.einc,sqrt(diag(self.cov_eunc)),'r--')
            #legend(['Total','Isotope','Ratio','TargetArealDensity','Reference','Eunc'],\
            #    bbox_to_anchor=(0., 1.02, 1., .102), loc=3,ncol=2, mode="expand", borderaxespad=0., fontsize=12)
            legend(['Total',r'$\delta$ E'],fontsize=12,loc='best')
        else:
            semilogx(self.einc,self.data_total['rel_unc']*100,'k.')
            #semilogx(self.einc,sqrt(self.cov_iso),'r+')
            ...
            semilogx(self.einc,sqrt(self.cov_eunc),'r<')
        #legend(['Total','Isotope','Ratio','TargetArealDensity','Reference','Eunc'],\
        legend(['Total',r'$\delta$ E'],fontsize=12,loc='best')
        xlabel('Incident Neutron Energy ('+self.einc_unit+')', fontsize=14)
        ylabel('Relative Uncertainties (%)', fontsize=14)
...
# n, gamma cross section clean ratio shape data ##################################    
class ngammacs_cleanratioshape(ngammacs):
    def __init__(self,general_info,data,unc_iso,reference,features):
        from numpy import sqrt, diag, array, shape, zeros
        from MatrixFunctions import get_cor
        from ToFCovariance import calculate_cov_eunc_RATIO
        ...
        # storing data in appropriate containers -----------------------------------------
        # loading data values, isotope uncertainties and general information in
        super(ngammacs_cleanratioshape,self).__init__(general_info=general_info,data=data,unc_iso=unc_iso,features=features)
        ...
        # calculating the different uncertainty types ------------------------------------        
        # WIP see if all units are the same, i.e. percent
        self.cov_iso = super(ngammacs_cleanratioshape,self).calculate_cov_iso()
        self.cov_eunc = calculate_cov_eunc_RATIO(self)
        self.cov_ratio = self.calculate_cov_ratio()
        self.cov_refnum = self.get_cov_refnum()
